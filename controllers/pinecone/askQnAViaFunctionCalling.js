...
const askQnAViaFunctionCalling = async (req, res) => {
  try {
    const result = await PineconeService.askQnAViaFunctionCalling(req.body.question, req.body.prompt)
    result.question = req.body.question
    res.sendJson({ type: __constants.RESPONSE_MESSAGES.SUCCESS, data: result })
  } catch (err) {
    console.log('askQnAViaFunctionCalling Error', err)
    return res.sendJson({
...
router.post('/askQnAViaFunctionCalling', validation, askQnAViaFunctionCalling)
