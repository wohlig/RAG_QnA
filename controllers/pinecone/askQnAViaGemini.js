...
const askQnAViaGemini = async (req, res) => {
  try {
    const result = await PineconeService.askQnAViaGemini(req.body.question, req.body.prompt, req.body.chatId)
    res.sendJson({ type: __constants.RESPONSE_MESSAGES.SUCCESS, data: result })
  } catch (err) {
    console.log('askQnAViaGemini Error', err)
    return res.sendJson({
...
router.post('/askQnAViaGemini', validation, askQnAViaGemini)
