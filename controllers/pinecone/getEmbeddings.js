...
const getEmbeddings = async (req, res) => {
  try {
    const result = await PineconeService.getEmbeddings(req.files)
    res.sendJson({ type: __constants.RESPONSE_MESSAGES.SUCCESS, data: result })
  } catch (err) {
    console.log('getEmbeddings Error', err)
    return res.sendJson({
...
router.post('/getEmbeddings', upload.array('files'), validation, getEmbeddings)
