...
//     const updateRange = `${range.split(':')[0]}:C${data.length + 1}`; // Adjust range to include new columns
//     const updatedValues = answers.map((answer, index) => [data[index][0], answer[1], answer[2]]);
...
const askQnaFromSheet = async (req, res) => {
  try {
    ...
    const updateRange = `B${startRow}:C${endRow}`;
    const updatedValues = data.map((row, index) => [answers[index][0], answers[index][1]]);
